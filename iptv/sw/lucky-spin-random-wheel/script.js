...
/* --------------- Size Of Each Piece  --------------------- */
const size = [35,30,10,10,10,10,5];
/* --------------- Background Colors  --------------------- */
var spinColors = [
  "#E74C3C",
  "#7D3C98",
...
const generateValue = (angleValue) => {
  for (let i of spinValues) {
    if (angleValue >= i.minDegree && angleValue <= i.maxDegree) {
      text.innerHTML = `<p contenteditable="true">الف مبروك , لقد حصلت على نسبة خصم  $${i.value}   </p>`;
...
spinBtn.addEventListener("click", () => {
  ...
  text.innerHTML = `<p contenteditable="true"> نتمنى لكم حظا موفقا</p>`;
